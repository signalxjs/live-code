# SignalX live-code — shared agent guide
...
SignalX `live-code` (`signalxjs/live-code`) — Monorepo for @sigx/live-code — interactive code blocks with a prebundled Monaco editor. A pnpm monorepo (ESM, `"type": "module"`) with the package under `packages/`. Tech stack: TypeScript (strict), Vitest, oxlint; published to npm under the `@sigx` scope.
...
Never commit straight to `main`.** Repo: `signalxjs/live-code`, base branch `main`.
...
4. **Open a PR with Copilot as the reviewer.** Reference the issue so it auto-closes
   ...
   ```sh
   gh api --method POST repos/signalxjs/live-code/pulls/<pr>/requested_reviewers \
...
5. **Wait for Copilot's review, then fix.** Do not merge before it has reviewed. Poll
   ...
   **Then resolve the threads.** The sigx-standard ruleset sets
   ...
   gh api graphql -f query='query { repository(owner:"signalxjs", name:"live-code") {
...
- `packages/live-code` → `@sigx/live-code` — Live code blocks for sigx documentation — interactive code examples with a prebundled Monaco editor.
...
4. Adapt the repo-specific sections of `AGENTS.md`: the intro (what the repo is),
   "Build, Test, Lint", and "Packages". Replace every `live-code` with the repo name.
5. Keep the workflow, worktree, and conventions sections as-is — they are the
   shared standard.
6. Lock down `main`: `node scripts/apply-branch-protection.mjs signalxjs/live-code`.
